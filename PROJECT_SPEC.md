...
- 左键单击 = 删除该张（与主页消耗牌/小丑牌/优惠券入口的左键移除统一；`contextmenu` 阻止浏览器菜单）；右键单击 = 打开扑克详情弹窗；编辑后统计、牌区张数、`playing_card/sort_id`（按新顺序重编 1..n）即时更新。
- **全站屏蔽浏览器右键菜单**（`main.ts` 的 `document` 级 `contextmenu` 监听）：右键在卡牌上是「查看详情」，菜单弹出会打断操作；仅输入框（`input`/`textarea`）内保留系统菜单，右键粘贴仍可用。
...
- 至少保留 1 张牌（0 张无法通过 `validateSave` 的「牌堆不能为空」）。全站不使用原生 `title` 悬浮提示（黑底白字气泡已全部移除），操作指引由界面文案与导出 toast 承担。

**消耗牌入口（`.consumables-entry`，v2 含负片开关与存档写入）**

- 位置：左列（`.switchers-col`）内、牌组与赌注选择**正下方**，宽度与牌组卡背 / 赌注筹码台的**框体**对齐（508px，居中于 612px 列内，不含两侧切换箭头）；主页 `.actions` 不再放消耗牌按钮。
- 内容一行一个含义：左「消耗牌 N / M」（N = 已选张数、M = 参数面板的「消耗牌」槽位数，实时读取），右「新增消耗牌」按钮（紫 `G.C.PURPLE`，打开图鉴弹窗）；下一行是一整块黑底牌区（`.ce-cards`，无牌时也保持 158px 高度），消耗牌**水平排列**，数量多时按可用宽度收缩步进（`--step`，默认 72px ⇒ 相邻重叠 40px，最小 24px）**部分叠在一起**，与牌组编辑弹窗的扑克行同一机制；不再画虚线空槽（空位数由「N / M」计数体现）。
- 交互：**左键点已有牌 = 移除该张**（toast 提示），是列表唯一的移除入口；槽位数被调小到低于已选数量时计数标红（`.ce-over`）并禁止继续添加，不自动删除已选牌。
- 槽位已满（`N = M`）时「新增消耗牌」按钮禁用置灰（`.ce-edit:disabled`）；移除一张或调大槽位数后自动恢复可用。
- 牌面 112×150（142:190 原比例）。这一版按需求把入口从右侧参数列挪到左列并放大牌面，代价是 1280×720 视口下页面整体出现**约 210px 纵向滚动**（5046b73 的单屏目标为此让位）；如需恢复单屏，可缩小牌面或压缩牌组 / 赌注区高度。

**消耗牌图鉴弹窗（`.consumables-modal`）**

- 入口：主页「新增消耗牌」按钮（槽位已满时该按钮禁用）。弹窗只做**图鉴**：不展示已有消耗牌列表（已有牌与其移除都在入口里）。
- 结构自上而下：分类标签（左缘与牌池框对齐，缩进一个箭头位 52px）/ 牌池（左右箭头 + 两行牌面，牌池为带描边的框体 `.cm-pages`）/ 页点 / 操作行（只有「返回」，蓝铺满）；无标题行与提示行，弹窗打开即见牌池。
- 分类标签 `塔罗牌 / 星球牌 / 幻灵牌`（默认塔罗牌，不做「全部」混合视图；选中态 = 分类色底 + 白字，不加描边）。
- 牌池为**固定版式分页**，不做滚动条：每页 2 行 × 每行 6 张（`PAGE_SIZE = 12`），左右各一个 `.arrow-btn` 翻页，下方页点（复用牌组选择的 `.dots/.dot`）标出页码；只有一页时翻页控件用 `visibility` 收起（保留占位，牌池宽度不变）。牌块宽度 `flex: 0 0 15.833%`（(100% − 5×1%) / 6），**行高用整行的 `aspect-ratio: 4.72/1` 锁死**——间距取百分比后行高只与行宽相关，因此空行也保持同样高度，翻页时弹窗不跳动。
- 交互：**左键点击牌面即选中**——立即加入工作列表、toast 提示并**自动关闭弹窗**（允许重复，与真机魔法牌组「愚者×2」一致）；已被选中的牌右下角显示 `×N` 角标；槽位满时不添加并提示「消耗牌槽位已满（M），可在右侧参数面板调大槽位数」。换分类标签回到第 1 页。
- 数量为**硬上限**（`core/consumables.ts:canAddConsumable`），上限实时取自参数面板，改了槽位数立刻生效。
- 「返回」= 不选任何牌直接关闭（与点遮罩关闭同一效果）；弹窗不再提供「重置」——牌组自带的起始消耗牌在切换牌组时由 `rebuildConsumables()` 自动重建，无需手动恢复。
- **说明框是悬停才浮出的独立框体**（`.cm-desc-pop`，绝对定位于牌池内、位置由 ts 按悬停牌动态计算：贴在该牌的左右两侧——右侧放不下自动换到左侧——顶部与牌对齐并夹在牌池边界内）：平时**完全不占位**，`pointer-events: none` 保证它不会抢走悬停目标（否则鼠标一进框体就会来回闪）。内容用 `renderDescLine(line, vars)` 渲染 `{C:xx}` 配色，`zhName` 按分类色 `--set-tarot/-planet/-spectral` 上色。`#N#` 取值由提取脚本按**开局状态**静态解析（等级 1、无小丑、概率基数 1），取不到的行整行省略（`c_temperance` 的当前小丑售价合计即此类）。
...
**✅ 已实现（v2）**：`core/saveDeck.ts:applyConsumablesToSave` 按附录 D 构建消耗牌卡表写进 `cardAreas.consumeables`（cards 替换 + card_count 同步；card_limit/temp_limit = 槽位数，由 applyRunInit 落盘）。生成结果与真机样例 `magic_initial.jkr` / `ghost_initial.jkr` 的消耗牌卡表**逐字段一致**（仅 sort_id 不同——实例计数器唯一即可，从全存档最大值续接），门禁见 `test/consumablesSave.test.ts`。要点（与附录 D 配合）：
...
- `ability` 数值按 `card.lua:277-307 set_ability` 从中心定义派生：`extra` 仅在 `config.extra` 存在时写（塔罗/星球无），**`ability.consumeable` = 整个 `center.config` 的深拷贝**（妖法 `{extra=2}`、愚者空表、星球 `{hand_type=...}`——附录 D.4 的「星球双写」说法过宽，以源码为准）；
- **魔法牌组**同时写 `used_vouchers.v_crystal_ball = true`（真机样例一致）：读档时游戏对 used_vouchers 逐个 redeem（水晶球 = 消耗区上限 +1），故 `starting_params.consumable_slots` 保持面板值（2），勿提前加进 card_limit；
- **负片开关（在图鉴弹窗内，按钮文案「负片开启/负片关闭」）**：开 = 牌池整卡按负片样式预览；**负片是逐牌属性**——加入时刻开关开着，这张牌就带负片（入口以负片样式渲染、导出写 `edition = { negative = true, type = "negative" }`），加入时开关关着则不带。开关状态存 `main.ts`，不随弹窗开关重置。⚠️ 负片 edition 结构无真机样例，依据 `Card:save` 的 edition 表与 playing card 同构（negative 无数值字段）。**负片每张使消耗区上限 +1**（`card.lua:405-417` set_edition：added_to_deck 的负片消耗牌令 `card_limit +1`；读档不走 set_edition，故存档直接写 +1 后的 card_limit）——HUD 计数 = `card_count/card_limit`（`cardarea.lua:265-266`，temp_limit = max(张数, card_limit)），例：2 槽 + 1 张负片显示 **1/3**；非负片张数仍 ≤ 槽位数（validate 断言）。
...
| `stake/` | 8 | 58×58 | v2 赌注 |
| `booster/` | 33 | — | 远期 |
| `icon/` | 16 | 132×132 | 未采用（多为空切片） |
| `logo/` | 2 | 666×432 | 未采用（页面标题改用纯文字） |
| `ui_asset/` | 8 | 36×36 | 花色/统计小图标，已复制 7 张（`{0,0}` 未使用） |
...
| 规则 | 依据 |
|---|---|
| 键集合固定：18 个键（见 D.1），无 `playing_card`/`seal`/`pinned`（nil 键不序列化）；负片消耗牌补 `edition = {negative=true, type="negative"}`（无数值字段；无真机样例，依据 `Card:save` 的 edition 表与扑克牌同构），其余版本/印章待 v2 再定 | 真机样例键集合 |
...
| `ability.order` / `base_cost` / `ability.effect` / `ability.extra` 等来自 `G.P_CENTERS` 对应定义 → 由数据提取脚本进 `src/data/`；`ability.consumeable` = 整个 center.config 深拷贝（`card.lua:305-307`，星球即 `{hand_type=...}`） | `card.lua:4659` Card:load |
| 开局态补充：`hands_played_at_create = 0`（真机参考存档里的 2 是局中购买所致）、`debuff = false`、`added_to_deck = true` | 样例对比 |
| 光谱类无 `effect` 时 ability 不写 effect 键；光谱类（妖法）`ability.extra` 与 `ability.consumeable.extra` 同值双写；塔罗类两处均无 `extra` 键；星球类无 `extra`（`consumeable.hand_type` 承载牌型） | 妖法样本 + `card.lua:305-307` |
